<script lang="ts">
	...
	function createKey(keyLike: string, options?: KeyOptions): KeyCell {
		...
		const label =
			options?.label ?? (normalized.trim() === "" ? "Space" : normalized);
		return {
			kind: "key",
			value: normalized,
			label,
	...
	const mainKeyboardRows: Array<{ cells: Array<KeyCell | SpacerCell> }> = [
		{
			cells: rowCells(
				...
				createSpacer(4),
				["F1", "F2", "F3", "F4"].map((label) =>
					createKey(label, { width: 2 }),
				),
				createSpacer(1),
				["F5", "F6", "F7", "F8"].map((label) =>
					createKey(label, { width: 2 }),
				),
				createSpacer(1),
				["F9", "F10", "F11", "F12"].map((label) =>
					createKey(label, { width: 2 }),
				),
				createSpacer(2),
				createKey("PrintScreen", {
					label: "PrtSc",
					width: 2,
					classes: ["nav"],
				}),
				createKey("ScrollLock", {
					label: "ScrLk",
					width: 2,
					classes: ["nav"],
				}),
		...
		},
		{
			cells: rowCells(
				...
				createKey("NumpadDivide", {
					label: "/",
					width: 2,
					classes: ["numpad"],
				}),
				createKey("NumpadMultiply", {
					label: "*",
					width: 2,
					classes: ["numpad"],
				}),
				createKey("NumpadSubtract", {
					label: "-",
					width: 2,
					classes: ["numpad"],
				}),
	...
	function buildSupplementalRows(totalColumns: number): KeyRow[] {
		const perRow = Math.ceil(
			supplementalKeys.length / supplementalRowCount,
		);
		const rows: KeyRow[] = [];
